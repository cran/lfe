...
# and a summary function which adjusts the summary.lm for the fixed effects
# obsolete?  Computations included in print.lm.fe.  That's not perhaps not the R-ish way
summary.felmfoo <- function(object,...) {
  # call summary.lm
  class(object) <- 'lm'
  z <- summary.lm(object,...)
  class(z) <- c('summary.felm',class(z))

  fe <- object$fe
  # total number of dummies
  # subtract one reference for each connection component
  # also, we've fiddled with the intercept, so we must
  # adjust for that one too
  conncomp <- nlevels(object$cfactor)
  numdum <- sum(unlist(lapply(fe,nlevels))) - conncomp
  # adjust the degrees of freedom, and sigma
  
  # degrees of freedom
  oldrdf <- z$df[2]
  z$df[1] <- z$df[1] + numdum
  z$df[2] <- z$df[2] - numdum
  z$df[3] <- z$df[3] + numdum
  rdf <- z$df[2]


  sefactor <- sqrt(oldrdf/rdf) 
  # standard errors

  z$coefficients[,2] <- z$coefficients[,2] * sefactor
  tval <- z$coefficients[,3]
  tval <- tval / sefactor
  pval <- 2*pt(abs(tval), rdf, lower.tail=FALSE)
  z$coefficients[,3] <- tval
  z$coefficients[,4] <- pval
    
  z$adj.r.squared <- 1-(1-z$adj.r.squared)*oldrdf/rdf

  z$fstatistic[1] <- z$fstatistic[1]*oldrdf/rdf
  z$fstatistic[2] <- z$fstatistic[2] + numdum
  z$fstatistic[3] <- rdf
 
  z$sigma <- z$sigma * sefactor
  return(z)
}
...
findfe <- function(dd,Rhs) {
  ...
  if(inherits(trysolve,'try-error')) {
    if(grepl('problem too large',geterrmessage())) {
      ...
      ch <- Cholesky(dd,super=FALSE,perm=FALSE)
...
print.lm.fe <- function(x,digits=max(3,getOption('digits')-3),...) {
  z <- x
  if(z$p == 0) {
    cat('(No coefficients)\n')
    return()
