...
static double kaczmarz(FACTOR *factors[], int e, mysize_t N, double *R,
                       double *x, double eps, mysize_t *work, int *stop,
                       LOCK_T lock) {
  /* The factors define a matrix D, we will solve Dx = R
     ...
     of adding a multiple of row i (with i = k %% N), the multiplying
     coefficient is (R[i] - (a_i,x))/e

     To get better memory locality, we create an (e X N)-matrix with the
     non-zero indices
  ...
  */
  ...
  do {
    mysize_t ie =
        0; /* equals i*e; integer multiplication is slow, keep track instead */
    double diff = 0.0;
    ...
    if (c >= 1.0 && iter > 20) {
      ...
      snprintf(
          buf, sizeof(buf),
          "Kaczmarz failed in iter %d*%d, c=1-%.0e, delta=%.1e, eps=%.1e\n",
...
static void *kaczmarz_thr(void *varg) {
  ...
  while (1) {
    ...
    (void)kaczmarz(arg->factors, arg->e, arg->N, arg->source[vecnum],
                   arg->target[vecnum], arg->eps, arg->work[myid], &arg->stop,
                   arg->lock);
  }
#ifndef NOTHREADS
#ifndef WIN
...
SEXP MY_kaczmarz(SEXP flist, SEXP vlist, SEXP Reps, SEXP initial, SEXP Rcores) {
  ...
  for (int i = 0; i < numfac; i++) sumlev += factors[i]->nlevels;
  ...
  for (int i = 0; i < listlen; i++) {
    ...
    if (!isMatrix(elt)) {
      if (LENGTH(elt) != N)
        error("Vector length (%d) must be equal to factor length (%d)",
              LENGTH(elt), N);
      numvec++;
    } else {
      if (nrows(elt) != N)
        error("Vector length must be equal to factor length %d %d ", nrows(elt),
              N);
      numvec += ncols(elt);
    }
  }
  ...
  // when allocating the work, we use mysize_t, but parts of it is accessed as
  // double which may be larger. So we allocate some more (8*sizeof(mysize_t)
  // more), and adjust the address so it's aligned on a double When using it,
  // make sure we do all the doubles first.

#ifdef NOTHREADS
  arg.work[0] = (mysize_t *)R_alloc(
      numfac * N + N * sizeof(double) / sizeof(mysize_t) + N + 2 * numfac + 8,
      sizeof(mysize_t));
  ...
  for (thr = 0; thr < numthr; thr++) {
    ...
    arg.work[thr] = (mysize_t *)R_alloc(
        numfac * N + N * sizeof(double) / sizeof(mysize_t) + N + 2 * numfac + 8,
        sizeof(mysize_t));
    uintptr_t amiss = (uintptr_t)arg.work[thr] % sizeof(double);
    if (amiss != 0)
      arg.work[thr] =
          (mysize_t *)((uintptr_t)arg.work[thr] + sizeof(double) - amiss);

#ifdef WIN
    threads[thr] =
        CreateThread(NULL, 0, kaczmarz_thr, &arg, 0, &threadids[thr]);
